...
class SerenityJob(SingleJob):
    ...
    def _get_ready(self):
        ...
        ams_job_dirs = [d for d in base_path.iterdir() if d.is_dir() and any(f.startswith('SerenityAMS') for f in os.listdir(d))]
    ...
    def _translate_AMSJob_to_Serenity(self, AMSJob_folder):
        ...
        convert_to_HDF5(
            AMSJob_folder / 'SerenityAMS.orbEnergies.bin',
            'SerenityAMS.orbEnergies.h5',
            'orbitalEnergies'
        )
