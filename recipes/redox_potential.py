...
def get_settings(molecule            :Molecule   = None,
    ...
    if not use_dftb:
        ...
        if spinpol == 0: unrestricted = 'No'
        else:            unrestricted = 'Yes'
...
def calculation_step(molecule            :Molecule,
    ...
    settings = get_settings(molecule=molecule,
                            task=task, 
                            use_dftb=use_dftb, 
...
def redox_potential(molecule            :Molecule, 
    ...
    if mode == 'oxidation':   Gelectron = -0.0375
    elif mode == 'reduction': Gelectron =  0.0375
    ...
    if method == 'DC':
        ...
        SP_ov_os = calculation_step(GO_ov['geometry'], task='SinglePoint', state=mode, phase=solvent, **general_settings)
        ...
        GO_nv    = calculation_step(molecule, frequencies=True, **general_settings)
        SP_nv_ns = calculation_step(GO_nv['geometry'], task='SinglePoint', use_COSMORS=True, solvent_path=COSMORS_solvent_path, **general_settings)
        ...
        GO_ov    = calculation_step(molecule, state=mode, frequencies=True, **general_settings)
        SP_ov_os = calculation_step(GO_ov['geometry'], task='SinglePoint', state=mode, use_COSMORS=True, solvent_path=COSMORS_solvent_path, **general_settings)
        GO_os    = calculation_step(molecule, state=mode, use_COSMORS=True, solvent_path=COSMORS_solvent_path, **general_settings)
        ...
        COSMO_nv = calculation_step(GO_nv['geometry'], task='SinglePoint', use_COSMORS=True, solvent_path=COSMORS_solvent_path, **general_settings)

        GO_ov    = calculation_step(molecule, state=mode, use_dftb=True, **general_settings)
        COSMO_ov = calculation_step(GO_ov['geometry'], task='SinglePoint', state=mode, use_COSMORS=True, solvent_path=COSMORS_solvent_path, **general_settings)
...
def main(mode):
    job_dir         = './Test'
    solvent         = 'DMSO'
    mol_files       = ['./molecules/20410-46-2.xyz', './molecules/109-75-1.xyz', './molecules/167951-80-6.xyz', './molecules/92-52-4.xyz', ]
    mol_names       = ['Phosphorodifluoridate (8CI,9CI)', '3-Butenenitrile', '3,3,3-Trifluoropropylene carbonate', '1,1-Biphenyl']
    ...
    for file, name, charge in zip(mol_files, mol_names, init_charges):
        ...
        for method in ['screening','TC-COSMO', 'TC-COSMO-RS', 'DC']:
            ...
            mol = Molecule(file)
            redoxpot = redox_potential(mol, mode, name=name, method=method, COSMORS_solvent_path=COSMORS_solvent_path, solvent=solvent, init_charge=charge)
