...
class Optimizer :
    ...
    def compute_energies (self, molecules, engine_settings=None, name='sp') :
        ...
        * ``engine_settings`` -- PLAMS Settings object:
        ...
        """
        if not self.use_pipe or len(molecules)==1 :
            molecules, energies, resultlist = self._perform_tasks_separately(molecules,engine_settings,name,'SinglePoint')
        else:
            molecules, energies, resultlist = self._perform_tasks_using_pipe (molecules,engine_settings,name,'SinglePoint')
        remaining_energies = [en for en in energies if not en is None]
