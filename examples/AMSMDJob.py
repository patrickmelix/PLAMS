...
def main():
    ...
    pre_eq = AMSNVTJob(settings=s, molecule=mol, name='pre_eq', temperature=400, timestep=1.0, thermostat='berendsen', nsteps=5)
    ...
    eq = AMSNVTJob.restart_from(pre_eq, settings=s, name='eq', nsteps=100, thermostat='NHC', samplingfreq=10, writevelocities=True)
    ...
    npt = AMSNPTJob.restart_from(eq, name='npt', nsteps=50, samplingfreq=10, pressure=2, thermostat='NHC', barostat='MTK')
