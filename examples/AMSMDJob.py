...
def main():
    ...
    pre_eq = NVTJob(settings=s, molecule=mol, name='pre_eq', temperature=400, timestep=1.0, thermostat='berendsen', nsteps=5)
    assert pre_eq.settings.input.ams.MolecularDynamics.Thermostat.Type == 'berendsen' #explicitly set above
    assert pre_eq.settings.input.ams.MolecularDynamics.Thermostat.Tau == 400 # default 400 * timestep if not set explicitly
    pre_eq.run()

    # Then switch to GAFF with an NHC thermostat
    s.input.forcefield.type = 'GAFF'
    s.input.forcefield.antechamberintegration = 'Yes'
    eq = NVTJob.restart_from(pre_eq, settings=s, name='eq', nsteps=100, thermostat='NHC', samplingfreq=10, writevelocities=True)
    ...
    npt = NPTJob.restart_from(eq, name='npt', nsteps=50, samplingfreq=10, pressure=2, thermostat='NHC', barostat='MTK')
    assert npt.settings.input.ams.MolecularDynamics.Thermostat.Temperature == 400 # inherited
    npt.run()

    # continue NVT
    nvt_from_npt = NVTJob.restart_from(npt, name='nvt_from_npt')
    ...
    # NVE from NPT
    nve_from_npt = NVEJob.restart_from(npt, name='nve_from_npt')
